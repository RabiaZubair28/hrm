...
class HrmisStaffSearch(http.Controller):
    ...
    def staff_search(self, **kwargs):
        ...
        # Restore from session if coming back
        if not kwargs and request.session.get('staff_search_filters'):
            kwargs = request.session.get('staff_search_filters')
        ...
        designation_groups = request.env['hrmis.designation'].sudo().read_group(
            [('active', '=', True)],
            ['name'],
            ['name'],
            orderby='name'
        ...
        designations = [d['name'] for d in designation_groups if d.get('name')]
